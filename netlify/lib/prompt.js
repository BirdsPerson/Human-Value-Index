// System prompt for the Assessment Engine. Lives server-side only: the browser
// never sees or sends it, so it cannot be swapped by a caller.
...
Return a JSON object of exactly this shape (the score and tier you report are a courtesy; the Value Index is recomputed from your breakdown using the formula above):
{
  "score": integer,
...
// Structured-output schema enforced by the API. Numeric bounds are applied
// server-side in shared/scoring.js rather than here.
export const RESULT_SCHEMA = {
  type: "object",
  additionalProperties: false,
  required: ["score", "tier", "breakdown", "verdict", "flags", "commendations"],
  properties: {
    score: { type: "integer" },
    tier: { type: "string" },
    breakdown: {
      type: "object",
      additionalProperties: false,
      required: ["utility", "honesty", "adaptability", "threat", "redundancy", "network", "alignment", "physical", "legacy"],
      properties: {
